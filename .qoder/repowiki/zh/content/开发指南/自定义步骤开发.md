...
## 更新摘要
**所做更改**   
- 新增 ContentItem 内容包装器类，支持字符串内容的双向数据绑定
- 添加 ContentItemCollectionConverter JSON 转换器，处理简单字符串数组和对象格式的内容结构
- 在 TypeTextFromFileStep 中集成新的 Contents 属性，支持从文件和 Excel 加载内容
- 更新序列化机制以支持新的内容数据结构
...
**更新** 新增了 ContentItem 内容包装器类和相应的 JSON 转换器，用于支持更灵活的内容管理和双向数据绑定。
...
- [ContentItem.cs:1-94](file://ShaoLu/Models/ContentItem.cs#L1-L94)
...
- 内容包装器与转换器
  - ContentItem 类提供字符串内容的双向数据绑定支持。
  - ContentItemCollectionConverter 处理 ObservableCollection<ContentItem> 的 JSON 序列化，兼容简单字符串数组和对象格式。
- 执行上下文
...
### 内容包装器与JSON转换器
**新增** ContentItem 类和相关转换器提供了强大的内容管理能力。

- ContentItem 包装器
  - 继承 ObservableObject，支持 WPF 双向数据绑定。
  - 提供 Text 属性和 ToString() 方法，简化 UI 绑定。
  - 支持空字符串默认值和构造函数重载。
- ContentItemCollectionConverter 转换器
  - 处理 ObservableCollection<ContentItem> 的 JSON 序列化。
  - 支持简单字符串数组格式：`["item1", "item2"]`
  - 支持对象格式：`[{"Text": "item1"}, {"Text": "item2"}]`
  - 向后兼容旧版字符串数组格式。
...
class ContentItemCollectionConverter {
+Read(ref Utf8JsonReader, Type, JsonSerializerOptions) ObservableCollection~ContentItem~
+Write(Utf8JsonWriter, ObservableCollection~ContentItem~, JsonSerializerOptions) void
}
...
ContentItemCollectionConverter --> ContentItem
TypeTextFromFileStep --> ContentItem
```

图表来源
- [ContentItem.cs:1-94](file://ShaoLu/Models/ContentItem.cs#L1-L94)
- [AutomationStep.cs:507-668](file://ShaoLu/Viewmodels/AutomationStep.cs#L507-L668)
...
- 内容集合转换器
  - ContentItemCollectionConverter 处理 Contents 属性的特殊序列化需求。
  - 支持多种输入格式，确保向后兼容性。
- 文件打包
...
participant CC as "ContentItemCollectionConverter"
...
CC-->>SF : 序列化Contents为字符串数组
SF-->>App : 完成保存
App->>SF : LoadFromAutoStepPackage(path)
SF->>FS : 解压zip到工作目录
SF->>Conv : Deserialize(stream, options含转换器)
Conv-->>SF : 返回具体步骤集合
CC-->>SF : 反序列化Contents集合
SF-->>App : 返回步骤集合
```

图表来源
- [AutomationStepJsonConverter.cs:1-119](file://ShaoLu/Converters/AutomationStepJsonConverter.cs#L1-L119)
- [ContentItem.cs:35-92](file://ShaoLu/Models/ContentItem.cs#L35-L92)
...
- 数据绑定
  ...
  - ContentItem 的 Text 属性支持直接在 UI 控件中编辑。
...
- 使用内容包装器
  - 对于需要管理多个字符串内容的步骤，考虑使用 ContentItem 和 ContentItemCollectionConverter。
...
CI["ContentItem"] --> CIConv["ContentItemCollectionConverter"]
TypeFromFile["TypeTextFromFileStep"] --> CI
```
...
- 内容集合优化
  - ContentItemCollectionConverter 使用流式处理，避免大量内存分配。
...
- 常见问题
  ...
  - 内容集合序列化问题：检查 Contents 属性是否正确标记 JsonConverter。
- 定位手段
  - 启用步骤日志，查看执行时间与输入内容。
  - 使用单元测试验证上下文解析与步骤克隆行为。
  - 检查 UI 模板是否正确绑定，命令是否触发。
  - 验证 ContentItem 的 Text 属性是否正确双向绑定。
...
## 结论
通过统一的基类设计、清晰的执行上下文、健壮的序列化机制与灵活的 UI 模板系统，本项目提供了完善的自定义步骤开发框架。新增的 ContentItem 内容包装器和转换器进一步增强了内容管理能力，使开发者可以更方便地处理复杂的文本输入场景。开发者只需关注具体业务逻辑实现，即可快速扩展新的自动化步骤，并保持与现有系统的无缝集成。
...
- 最佳实践
  ...
  - 为新步骤类型补充模板与注册映射。
  - 对于需要管理多个字符串内容的步骤，优先使用 ContentItem 和 ContentItemCollectionConverter。
- 调试技巧
  - 使用断点观察 IsTrue/IsError/ErrorType 变化。
  - 打印执行上下文中的结果，验证条件解析。
  - 逐步缩小问题范围，先验证基础功能再扩展复杂逻辑。
  - 检查 ContentItem 集合的序列化和反序列化过程。